...
class VAE_(nn.Module):
    ...
    def encode(self, x, scale):
        ...
        if (t - 1) % self.temporal_window:
            self._enc_conv_idx = [0]
            # Calculate remainder slice
            remainder_start = 1 + self.temporal_window * (iter_ - 1)
            remainder_frames = t - remainder_start

            # Ensure at least 4 frames for temporal convolutions
            if remainder_frames < 4:
                remainder_start = max(1, t - 4)
            ...
            # Calculate overlap and only concatenate new frames
            original_remainder_start = 1 + self.temporal_window * (iter_ - 1)
            overlap = original_remainder_start - remainder_start

            if overlap > 0:
                # Skip overlapping frames from encoder output
                out = torch.cat([out, out_[:, :, overlap:, :, :]], 2)
            else:
                out = torch.cat([out, out_], 2)
