...
class DiT(nn.Module):
    def __init__(
        ...
        if self.enable_confidence_pred:
            # create the embedding MLP for the error threshold
            self.err_thresh_mlp = nn.Sequential(
                nn.Linear(256, dim, bias=True),
                nn.SiLU(),
                nn.Linear(dim, dim, bias=True),
            )
            
            # create the confidence prediction head
            self.conf_blocks = nn.ModuleList(
                [Block(dim, num_heads, rope_config) for _ in range(num_layers_conf_head)]
            )
            
            # final layer for the confidence head
            self.final_layer_conf_head = FinalLayer(dim, patch_size, in_channels)
    ...
    def initialize_weights(self) -> None:
        ...
        # Confidence head
        if self.enable_confidence_pred:
            # Initialize the error threshold embedding MLP:
            nn.init.normal_(self.err_thresh_mlp[0].weight, std=0.02)
            nn.init.normal_(self.err_thresh_mlp[2].weight, std=0.02)
            ...
            # Zero-out output layers:
            nn.init.constant_(self.final_layer_conf_head.adaLN_modulation[-1].weight, 0)
            nn.init.constant_(self.final_layer_conf_head.adaLN_modulation[-1].bias, 0)
            nn.init.constant_(self.final_layer_conf_head.linear.weight, 0)
            nn.init.constant_(self.final_layer_conf_head.linear.bias, 0)
    ...
    def get_cond_err_threshold(self, err_thresh: torch.tensor) -> torch.Tensor:
        """
        Compute embeddings for error threshold in confidence prediction
        """
        T = err_thresh.shape[1]
        err_thresh = einops.rearrange(err_thresh, "b t -> (b t)")
        err_thresh_freq = self.timestep_embedding(err_thresh)
        c = self.err_thresh_mlp(err_thresh_freq)
        c = einops.rearrange(c, "(b t) d -> b t d", t=T)
        return c
    ...
    def forward(
        ...
        # clone the prediction for confidence prediction head
        if self.enable_confidence_pred:
            # processed inputs and condition
            c_conf = c.detach().clone()
            x_conf = x.detach().clone()
            
            # compute the error threshold embedding
            c_conf_err_thresh = self.get_cond_err_threshold(err_threshold_conf_pred)
            
            # sum conditioning embeddings
            c_conf = c_conf + c_conf_err_thresh
        ...
        if self.enable_confidence_pred:
            # forward pass on the confidence head with conditioning
            for block in self.conf_blocks:
                x_conf = block(x_conf, c_conf)
                
            # final layer
            x_conf = self.final_layer_conf_head(x_conf, c_conf)
            x_conf = einops.rearrange(x_conf, "b t h w d -> (b t) h w d")
            x_conf = self.unpatchify(x_conf)
            x_conf = einops.rearrange(x_conf, "(b t) h w c -> b t h w c", t=T)
        else:
